...
type FakeOscillator = OscillatorNode & { start: ReturnType<typeof vi.fn>; stop: ReturnType<typeof vi.fn> };
type FakeGain = GainNode & { gain: AudioParam & { setValueAtTime: ReturnType<typeof vi.fn>; exponentialRampToValueAtTime: ReturnType<typeof vi.fn> } };
...
function makeContext(state: AudioContextState = 'running'): FakeContext {
  ...
  const context = {
    ...
    get currentTime() { return context.now; },
...
describe('AudioController', () => {
  ...
  it('schedules metronome beats against the audio clock without duplicate scheduling', () => {
    ...
    const controller = new AudioController({
      ...
      setIntervalFn: ((callback: () => void) => { intervals.push(callback); return 1; }) as typeof setInterval,
  ...
  it('keeps cue playback separate and gives each cue its own pitch pattern', () => {
    ...
    expect(context.oscillators.map((oscillator) => oscillator.frequency.value)).toEqual([660, 880, 1040]);
  });
