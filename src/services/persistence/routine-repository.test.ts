...
import { LocalStorageRoutineRepository, migrateRoutine, ROUTINE_STORAGE_KEY } from "./routine-repository";
...
describe("LocalStorageRoutineRepository", () => {
  ...
  it("migrates legacy exercises without losing their IDs, order, values, or settings", () => {
    const migrated = migrateRoutine({
      schemaVersion: 1, id: "old", name: "Old routine", defaultBreakDurationSec: 30,
      warningLeadTimeSec: 20, autoAdvance: true, updatedAt: "2026-01-01T00:00:00.000Z",
      exercises: [
        { id: "first", title: "Scales", tempoBpm: 80, durationSec: 300 },
        { id: "second", title: "Free", tempoBpm: null, durationSec: null },
      ],
    });
    expect(migrated).toMatchObject({ schemaVersion: 2, quickRestDurationSec: 30, warningLeadTimeSec: 20, metronomeSound: "classic" });
