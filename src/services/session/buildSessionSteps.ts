...
function assertRoutineSettings(routine: Routine): void {
  if (!Number.isFinite(routine.defaultBreakDurationSec) || routine.defaultBreakDurationSec < 0 || routine.defaultBreakDurationSec > 30 * 60) {
...
function assertExercise(exercise: Routine["exercises"][number]): void {
  ...
  if (exercise.tempoBpm !== null && (!Number.isFinite(exercise.tempoBpm) || exercise.tempoBpm < 20 || exercise.tempoBpm > 300)) {
    throw new Error("Tempo must be between 20 and 300 BPM.");
  }
  if (exercise.durationSec !== null && (!Number.isFinite(exercise.durationSec) || exercise.durationSec < 1 || exercise.durationSec > 120 * 60)) {
