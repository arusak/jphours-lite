...
describe("practice configuration", () => {
  it("exposes the YAML-defined defaults and bounds", () => {
    ...
    expect(practiceConfig.quickRestDuration).toMatchObject({ default: 30, min: 0, max: 180, increment: 5 });
  ...
  it("rejects invalid policy bounds at the application boundary", () => {
    expect(() => validatePracticeConfig({ tempo: { default: 10, min: 20, max: 30, increment: 1 } })).toThrow();
  });
});
