...
describe("routine validation", () => {
  it("does not allow a tempo without a duration", () => {
    expect(validateExercise(createExercise({ title: "Scales", tempoBpm: 100 })).durationSec).toBe("A metronome exercise needs a duration.");
  });

  it("accepts an open-ended exercise and a zero-second break", () => {
    expect(isRoutineValid(createRoutine({ defaultBreakDurationSec: 0, exercises: [createExercise({ title: "Explore" })] }))).toBe(true);
  });
});
