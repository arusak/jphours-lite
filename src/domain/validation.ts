...
export function validateExercise(
  exercise: Exercise,
): FieldErrors<"title" | "tempoBpm" | "durationSec"> {
  ...
  if (
    ...
    (!Number.isInteger(exercise.durationSec) ||
      exercise.durationSec < MIN_DURATION_SEC ||
      exercise.durationSec > MAX_DURATION_SEC)
  ) {
...
> & {
  ...
  const exercisesById = Object.fromEntries(
    routine.exercises.map((exercise) => [exercise.id, validateExercise(exercise)]),
  );
  const errors: FieldErrors<"exercises" | "defaultBreakDurationSec"> & {
    exercisesById: Record<string, ReturnType<typeof validateExercise>>;
  } = { exercisesById };
  if (routine.exercises.length === 0) errors.exercises = "Add at least one exercise.";
  if (
    !Number.isInteger(routine.defaultBreakDurationSec) ||
    routine.defaultBreakDurationSec < MIN_BREAK_DURATION_SEC ||
    routine.defaultBreakDurationSec > MAX_BREAK_DURATION_SEC
  ) {
...
export function isRoutineValid(routine: Routine): boolean {
  ...
  return (
    ...
    Object.values(validation.exercisesById).every((errors) => Object.keys(errors).length === 0)
  );
}
