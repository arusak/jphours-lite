...
export function validateExercise(exercise: Exercise): EntryErrors {
  ...
  if (exercise.durationSec !== null && !inPolicy(exercise.durationSec, practiceConfig.exerciseDuration)) errors.durationSec = "Enter a duration in the configured range.";
  return errors;
}

export function validateBreak(entry: Break): EntryErrors {
  const errors: EntryErrors = {};
  if (!inPolicy(entry.durationSec, practiceConfig.breakDuration)) errors.durationSec = "Enter a break duration in the configured range.";
  return errors;
}
...
export function validateRoutine(routine: Routine): FieldErrors<"entries" | "quickRestDurationSec" | "warningLeadTimeSec" | "metronomeSound"> & { entriesById: Record<string, EntryErrors> } {
  const errors: FieldErrors<"entries" | "quickRestDurationSec" | "warningLeadTimeSec" | "metronomeSound"> & { entriesById: Record<string, EntryErrors> } = { entriesById: Object.fromEntries(routine.entries.map((entry) => [entry.id, validateEntry(entry)])) };
...
export function isRoutineValid(routine: Routine): boolean {
  ...
  return !validation.entries && !validation.quickRestDurationSec && !validation.warningLeadTimeSec && !validation.metronomeSound && Object.values(validation.entriesById).every((errors) => Object.keys(errors).length === 0);
}
